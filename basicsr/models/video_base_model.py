...
class VideoBaseModel(SRModel):
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        self.metric_results = torch.zeros(500, len(self.opt['val']['metrics']), dtype=torch.float32, device='cuda')
        ...
        if with_metrics:
            ...
            if rank == 0:
                best = self._log_validation_metric_values(current_iter, dataset_name, tb_logger)
                if best:
                    self.save_net()
                    self.save_training_state(current_iter)


@MODEL_REGISTRY.register()
class VideoBaseMode(SRModel):
    """Base video SR model."""
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        dataset = dataloader.dataset
        dataset_name = dataset.opt['name']
    ...
    def save_net(self, param_key='params'):
        """Save networks.
        ...
        best = 'best'
        net_label = 'net_g'
        save_filename = f'{net_label}_{best}.pth'
    ...
    def save_training_state(self, current_iter):
        ...
        """
        if current_iter != -1:
            state = {'iter': current_iter, 'optimizers': [], 'schedulers': []}
    ...
    def _log_validation_metric_values(self, current_iter, dataset_name, tb_logger):
        ...
        return value == self.best_metric_results[dataset_name][metric]["val"]