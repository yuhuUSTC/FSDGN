...
class REVIDEDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(REVIDEDataset, self).__init__()
        self.opt = opt
        ...
        with open(opt['meta_info_file'], 'r') as fin:
            for line in fin:
                ...
                self.num_pframe[folder] = int(frame_num)
        ...
        logger.info(f'Temporal augmentation interval list: [{interval_str}]; 'f'random reverse is {self.random_reverse}.')
    ...
    def __getitem__(self, index):
        ...
        while (start_frame_idx < 0) or (end_frame_idx > self.num_pframe[clip_name]-1):
            center_frame_idx = random.randint(1, self.num_pframe[clip_name]-2)
            start_frame_idx = (center_frame_idx - self.num_half_frames * interval)
            end_frame_idx = center_frame_idx + self.num_half_frames * interval
        ...
        if self.flow_root is not None:
            ...
            for i in range(self.num_half_frames, 0, -1):
                if self.is_lmdb:
                    ...
                    flow_path = (self.flow_root / clip_name / f'{frame_name}_p{i}.JPG')
            ...
            for i in range(1, self.num_half_frames + 1):
                if self.is_lmdb:
                    ...
                    flow_path = (self.flow_root / clip_name / f'{frame_name}_n{i}.JPG')
                img_bytes = self.file_client.get(flow_path, 'flow')
                cat_flow = imfrombytes(img_bytes, flag='grayscale', float32=False)  # uint8, [0, 255]
                dx, dy = np.split(cat_flow, 2, axis=0)
                flow = dequantize_flow(dx, dy, max_val=20, denorm=False)  # we use max_val 20 here.
...
@DATASET_REGISTRY.register()
class REVIDERecurrentDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(REVIDERecurrentDataset, self).__init__()
        self.opt = opt
        self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(opt['dataroot_lq'])
        self.num_frame = opt['num_frame']
